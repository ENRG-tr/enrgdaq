...
class TestDAQJobHandleAlerts(unittest.TestCase):
    ...
    def test_handle_message(self, mock_get_unix_timestamp_ms):
        ...
        message.daq_job_info.daq_job_type = "TestJob"  # Use a real string value
        ...
        self.assertIsInstance(stored_message, DAQJobMessageStorePyArrow)
        # Verify the table structure
        table = stored_message.get_table()
        self.assertEqual(
            table.column_names, ["timestamp", "daq_job", "severity", "message"]
        )
        self.assertEqual(table.num_rows, 1)
        stored_message.release()
